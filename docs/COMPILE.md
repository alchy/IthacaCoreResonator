...
Banka je JSON soubor vygenerovaný z WAV sample banku nástrojem pro extrakci parametrů
(v separátním Python projektu `resonators`). Po extrakci stačí soubor zkopírovat:

```bat
:: Zkopírovat hotový JSON do soundbanks/
copy <cesta_k_params.json> soundbanks\salamander.json
```

Na Linuxu/macOS:
```bash
cp <cesta_k_params.json> soundbanks/salamander.json
```

Zdrojový WAV bank: `C:/SoundBanks/IthacaPlayer/ks-grand/` (Salamander Grand Piano, CC0).
Extrakce parametrů: viz projekt `resonators/analysis/extract-params.py`.
