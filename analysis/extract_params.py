...
Adapted from alchy/resonators for IthacaCoreResonator.
...
Extracts per-note/velocity:
  ...
  - n_strings per note (1/2/3 — standard piano stringing)
  - Longitudinal (phantom) partials for bass notes (MIDI < 50, f ≈ 2·f_k)

Output:  analysis/params-ks-grand.json
...
def _setup_log() -> None:
    log_dir = Path("analysis/runtime-logs")
...
def n_strings_for_midi(midi: int) -> int:
    """Standard piano stringing (matches note_lut.cpp fallback thresholds)."""
    if midi <= 27:  return 1
    if midi <= 48:  return 2
    return 3


# ── Longitudinal (phantom) partial detection ──────────────────────────────────

def detect_longitudinal_partials(
    freqs: np.ndarray, spec: np.ndarray,
    transverse_peaks: list[dict], f0_fit: float, B: float, sr: int,
    stft_times: np.ndarray, stft_freqs: np.ndarray, stft_mag: np.ndarray,
) -> list[dict]:
    """
    Detect longitudinal (phantom) partials at f ≈ 2·f_k.

    Physics: geometric nonlinearity couples transverse and longitudinal
    string vibrations. Longitudinal modes appear at f_L ≈ 2·f_k with
    decay τ_L ≈ τ/2. Most prominent in bass (MIDI < 50) at forte.
    Paper refs: Chabassier/Chaigne/Joly 2013, RR-8181, Simionato 2024.

    Returns list of partial dicts with is_longitudinal=True, ready to be
    appended to the transverse partials list (C++ renders them identically
    to transverse mono partials — no special path needed).
    """
    long_partials = []
    transverse_freqs = {p['f_measured'] for p in transverse_peaks}

    for p in transverse_peaks[:12]:  # only first 12 bass partials matter
        f_trans = p['f_measured']
        f_long_expected = 2.0 * f_trans
        if f_long_expected >= sr / 2 * 0.97:
            continue

        f_peak, amp = find_peaks_near(freqs, spec, f_long_expected, width_frac=0.02)
        if amp < 1e-12:
            continue

        # Must stand above local noise floor (SNR > 3)
        ...
        # Must not coincide with a transverse partial (within 0.5%)
        if any(abs(f_peak - tf) / max(tf, 1.0) < 0.005 for tf in transverse_freqs):
            continue

        # Fit decay from STFT envelope (τ_L should be ~τ_k/2, faster than transverse)
        tau_long = None
        if stft_mag.shape[0] >= 8:
            t_env, a_env = partial_envelope_from_stft(stft_times, stft_freqs, stft_mag, f_peak)
            if len(t_env) >= 12:
                i_pk = find_peak_frame(a_env)
                dec = fit_decay(t_env, a_env, i_pk)
                tau_long = dec['tau1']

        long_partials.append({
            'k':              p['k'],   # parent transverse partial index
            'f_hz':           float(f_peak),
            'A0':             float(amp),
            'tau1':           tau_long,
            'tau2':           None,
...
def analyze_file(path: str, midi: int, vel: int, verbose: bool = False) -> dict:
    ...
    result = {
        ...
        'n_strings': n_strings_for_midi(midi),
        'n_partials': 0,
        'partials': [],
        'noise': {},
    }
    ...
    # Detect longitudinal (phantom) partials for bass notes only
    if midi < 50 and stft_mag.shape[0] >= 8:
        long_parts = detect_longitudinal_partials(
        ...
        if long_parts:
            ...
            if verbose:
                print(f"    longitudinal partials detected: {len(long_parts)}")
...
def main():
    ...
    parser.add_argument('--out', default='analysis/params-ks-grand.json')
